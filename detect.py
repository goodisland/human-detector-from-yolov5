...
def sendMail(to_address:str, dirpath:list, mail_body:str):
    # SMTPサーバの設定
    host = "smtp.gmail.com"
    port = 587
    ...
    for filepath in dirpath:
        with open(filepath, "rb") as f:
            image = MIMEImage(f.read(), name=os.path.basename(filepath))
            msg.attach(image)
    ...
    print(f"Send email to {to_address}.")
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    # Ryo Shimada Changed
                    if count_person > 0:
                        mail_body += f"<li>{count_person} person{'s' * (count_person > 1)} detected from {path}</li>"
                        path_img.append(save_path)
